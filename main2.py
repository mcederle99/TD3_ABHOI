...
def eval_policy(policy, env_name, seed, eval_episodes=10):
	...
	for q in range(eval_episodes):
		...
		while not done:
			...
			old_state = state[:int(state_dim / 2)]
			state, reward, _, done, _ = eval_env.step(action)
			state = np.concatenate((old_state, state - old_state))
			avg_reward += reward
...
if __name__ == "__main__":
	...
	file_name = f"{args.policy}_new_{args.env}_{args.seed}"
	print("---------------------------------------")
	...
	state_dim = int(env.observation_space.shape[0] * 2)
	...
	for t in range(int(args.max_timesteps)):
		...
		next_state = np.concatenate((old_state, next_state - old_state))
